import { getRepository, MoreThan, Like,Not } from 'typeorm';
...
export const getAllPersonnel = async (input) => {
  try {
    ...
    const condition = {
      ...
      // lv: { id: Number.parseInt(input.level) || MoreThan(0) },
      // ctcv: { cv: Number.parseInt(input.position) || MoreThan(0) },
    };
    const [personnel, count] = await Promise.all([
      repo.find({
